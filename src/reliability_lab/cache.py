...
# ---------------------------------------------------------------------------
# Shared utilities â€” use these in both ResponseCache and SharedRedisCache
# ---------------------------------------------------------------------------
...
class ResponseCache:
    ...
    def similarity(a: str, b: str) -> float:
        ...
        2. Tokenize both strings: split into words + character n-grams (n=3)
           e.g., "hello world" â†’ ["hello", "world", "hel", "ell", "llo", "wor", "orl", "rld"]
...
class SharedRedisCache:
    ...
    Data model (suggested):
        ...
        TTL    = Redis EXPIRE (automatic cleanup â€” no manual eviction)
    ...
    Provided helpers:
        _is_uncacheable(query)          â€” True if privacy-sensitive
        _looks_like_false_hit(q, key)   â€” True if 4-digit numbers differ
        self._query_hash(query)         â€” deterministic short hash for Redis key
        ResponseCache.similarity(a, b)  â€” reuse your improved similarity function
    """
    ...
    def ping(self) -> bool:
        ...
        except Exception:  # noqa: BLE001 - connectivity failure should degrade to a cache miss
            return False
    ...
    def get(self, query: str) -> tuple[str | None, float]:
        ...
        3. Try self._redis.hget(key, "response") â€” if found return (response, 1.0)
        ...
        for key in self._redis.scan_iter(f"{self.prefix}*"):
            ...
            if score > best_score:
                best_query, best_response, best_score = str(cached_query), str(cached_response), score
